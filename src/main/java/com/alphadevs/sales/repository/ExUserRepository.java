...
public interface ExUserRepository extends JpaRepository<ExUser, Long>, JpaSpecificationExecutor<ExUser> {
    ...
    @Query(value = "select distinct exUser from ExUser exUser left join fetch exUser.locations left join fetch exUser.userGroups",
        countQuery = "select count(distinct exUser) from ExUser exUser")
    Page<ExUser> findAllWithEagerRelationships(Pageable pageable);

    @Query("select distinct exUser from ExUser exUser left join fetch exUser.locations left join fetch exUser.userGroups")
    List<ExUser> findAllWithEagerRelationships();

    @Query("select exUser from ExUser exUser left join fetch exUser.locations left join fetch exUser.userGroups where exUser.id =:id")
