...
export class DocumentNumberConfigResolve implements Resolve<IDocumentNumberConfig> {
  constructor(private service: DocumentNumberConfigService) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IDocumentNumberConfig> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(map((documentNumberConfig: HttpResponse<DocumentNumberConfig>) => documentNumberConfig.body));
    }
