...
public class DocumentHistoryResourceIT {
    ...
    private static final LocalDate DEFAULT_HISTORY_DATE = LocalDate.ofEpochDay(0L);
    private static final LocalDate UPDATED_HISTORY_DATE = LocalDate.now(ZoneId.systemDefault());
    private static final LocalDate SMALLER_HISTORY_DATE = LocalDate.ofEpochDay(-1L);
    ...
    public void getAllDocumentHistories() throws Exception {
        ...
        restDocumentHistoryMockMvc.perform(get("/api/document-histories?sort=id,desc"))
            ...
            .andExpect(jsonPath("$.[*].historyDate").value(hasItem(DEFAULT_HISTORY_DATE.toString())));
    }
    ...
    public void getDocumentHistory() throws Exception {
        ...
        restDocumentHistoryMockMvc.perform(get("/api/document-histories/{id}", documentHistory.getId()))
            ...
            .andExpect(jsonPath("$.historyDate").value(DEFAULT_HISTORY_DATE.toString()));
    }
    ...
    @Test
    @Transactional
    public void getAllDocumentHistoriesByUserIsEqualToSomething() throws Exception {
        // Get already existing entity
        ExUser user = documentHistory.getUser();
        documentHistoryRepository.saveAndFlush(documentHistory);
        Long userId = user.getId();

        // Get all the documentHistoryList where user equals to userId
        defaultDocumentHistoryShouldBeFound("userId.equals=" + userId);

        // Get all the documentHistoryList where user equals to userId + 1
        defaultDocumentHistoryShouldNotBeFound("userId.equals=" + (userId + 1));
    }
    ...
    public void getAllDocumentHistoriesByChangeLogIsEqualToSomething() throws Exception {
        ...
        em.flush();
        documentHistory.setChangeLog(changeLog);
        documentHistoryRepository.saveAndFlush(documentHistory);
